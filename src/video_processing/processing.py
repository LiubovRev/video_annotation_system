...
START_TRIM_SEC = 0
END_TRIM_SEC = int(60*28)

X_MIN, Y_MIN = 40, 40
X_MAX, Y_MAX = 1240, 700
RESIZE_W, RESIZE_H = None, None

POSE_MASK_THRESHOLD = 0.1  # include smaller subjects
...
def postprocess_poses(poses_dir: Path):
    """Load raw keypoints from npz/json, smooth and fill missing joints."""
    for f in poses_dir.glob("*.npz"):
        data = np.load(f)
        keypoints = data["keypoints"]  # shape: (frames, joints, 3)
...
def main():
    ...
    run([
        ...
        "--mask_threshold", str(POSE_MASK_THRESHOLD),
